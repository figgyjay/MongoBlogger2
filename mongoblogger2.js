const uuidList = [
  ...
  */
  ...
  // const postId = '35a04237-507c-45f5-b770-3ce2c2fd088b' // This variable should be the uuid of the blog post you created before using createPost
  ...
  // updatePost(postsForUpdate[0].id, updatedPost)
  
  /*
  Requirements:
  - Add code in updatePost so that Mongo will find the post you created when you ran createPost and update that post to the new data 
      coming through in the newPostData parameter. 
  ...
  */
  ...
  const postIdToDelete = "81dac8b1-06c7-40f9-94c7-ceeefa3a3475";
  
  // Uncomment the following line when you're ready to run deletePost
  // deletePost(postIdToDelete)
  
  const deletePosts = (deleteManyArray) => {
    db.posts.deleteMany({
      id: {
        $in: deleteManyArray,
              // $in: uuidList
  ...
  const searchObjectForDelete = {
    author: "Bruce Wayne",
  };
  const sortObjectForDelete = {
    lastModified: 1,
  };
  const limitForDelete = 10;
  const skipForDelete = 0;
  
  // Uncomment the following line when you're ready to run getPosts
  const postsForDelete = getPosts(
    searchObjectForUpdate,
    sortObjectForUpdate,
    limitForUpdate,
    skipForUpdate
  ...
  for (let i = 0; i < postsForDelete.length; i++) {
    postIdsToDelete.push(postsForDelete[i].id);
  }
